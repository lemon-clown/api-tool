...
/**
 * @member dataFileRootPath   mock 数据文件所在的根目录
 * @member prefixUrl          请求的 url 前缀
 * @member useDataFileFirst   是否优先使用数据文件作为 mock 数据源
 */
interface Params {
 ...
 */
export function dataFileMock({ dataFileRootPath, prefixUrl, useDataFileFirst }: Params) {
  return async function (ctx: Koa.Context, next: Function) {
    // 以数据文件作为 mock 数据源
    const loadFromDataFile = async () => {
      let u = ctx.path.startsWith(prefixUrl) ? ctx.path.slice(prefixUrl.length) : ctx.path
      u = u.replace(/^[/\\]+/, '').replace(/[/\\]+$/, '')
      const dataPath = path.join(dataFileRootPath, u)
    ...
    // 调整 mock 数据源的优先顺序
    const methods = useDataFileFirst ? [loadFromDataFile, loadFromNext] : [loadFromNext, loadFromDataFile]
